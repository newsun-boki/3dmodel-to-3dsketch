...
def hough3(p,cart,num_point_threshold,point_distance_threshold,merge_angle_point):
    ...
    pbar.close()
    acc_l = len(acc[0])
    # acc = [np.array(x) for x in acc if len(x) == acc_l]
    acc=np.array(acc)
...
def make_image(line3,k_centers,point_index,make_gif,name):
    ...
    for i in range(k_centers.shape[0]):
        ...
        line2=ax.plot3D(l[:,0], l[:,1], l[:,2],c='k')#colormap[i])
        pred_line3.append(l)
...
# 加载点云数据
pcd = o3d.io.read_point_cloud("combined3.ply")  # 例如PLY文件
# 将点云转换为NumPy数组
# line3= np.asarray(pcd.points)
...
make_image(line3,k_centers,point_index,False,'hough.png')